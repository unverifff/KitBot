...
qJointGuess{4} = [-0.75, deg2rad(165), pi/4, deg2rad(65), deg2rad(-20), -pi/2, 0];
qJointGuess{5} = [-1.3, deg2rad(165), pi/4, deg2rad(65), deg2rad(-20), -pi/2, 0];
qJointGuess{6} = [0, deg2rad(165), pi/4, deg2rad(65), deg2rad(-20), -pi/2, 0];
...
qRight = [0, pi, pi/4, pi/4, 0, -pi/2, 0];
qLeft = [0, 0, pi/4, pi/4, 0, -pi/2, 0]; % Need to fix the Joints to match qGuess
...
function trapTraj(robotName, steps, initQ, nextQ, itemName, itemIndex)
    % Creat Joint Trajectories
    s = lspb(0, 1, steps);
    qMatrix = nan(steps, 7);
    for p = 1:steps
    ...
    % Add Resolve Rate Motion Control Here
    ...
    for n = 1:length(qMatrix)
        ...
        if nargin >= 5 && ~isempty(itemName) && nargin >= 6 && ~isempty(itemIndex)
            ...
            itemName.robotModel{itemIndex}.base = endEffectorLocation.T  * transl(0, 0, 0) * trotx(pi/2) * troty(0) * trotz(0);
